...
type ShardKV struct {
	...
	make_end func(string) *labrpc.ClientEnd // function to create a client end to communicate with other groups
...
func (kv *ShardKV) migrationAction() {
	...
	for gid, shardIDs := range gid2Shards {
		DPrintf("{节点 %v}{组 %v} 开始一个拉取任务，从组 %v 获取分片 %v，当前配置为 %v", kv.rf.GetId(), kv.gid, gid, shardIDs, kv.currentConfig)
		wg.Add(1)
		go func(servers []string, configNum int, shardIDs []int) {
			...
			for _, server := range servers {
				...
				if srv.Call("ShardKV.GetShardsData", &pullTaskArgs, pullTaskReply) && pullTaskReply.Err == OK {
					// 从这些服务器拉取数据
					DPrintf("{节点 %v}{组 %v} 获取到一个拉取任务回复 %v，并尝试在当前配置号为 %v 时提交它", kv.rf.GetId(), kv.gid, pullTaskReply, configNum)
